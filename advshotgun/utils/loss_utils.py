...
def norm_lp_loss(adv_pc:torch.Tensor, ori_pc:torch.Tensor, p:float=2.0):
    # if adv_pc is None:
    #     return partial(norm_lp_loss, ori_pc=ori_pc, p=p)
    
    ln_norms = torch.norm((adv_pc - ori_pc).view(adv_pc.size(0), -1), dim=1, p=p)
    return ln_norms

def CWLoss(logits, target, kappa:float=0, tar:bool=False, num_classes:int=40, top5_attack:bool=False):
...
#     x_ = torch.einsum('bij,bjk->bik',v.transpose(1,2), ori_pc.transpose(1, 2)) # (b,n,3)
#     x_adv = torch.einsum('bij,bjk->bik',v.transpose(1,2), adv_pc.transpose(1, 2)) # (b,n,3)
